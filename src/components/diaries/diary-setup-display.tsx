...
function ListSection({
  ...
  return (
    <div
      className="relative overflow-hidden rounded-2xl border border-white/[0.06] bg-gradient-to-br from-white/[0.04] to-transparent p-4 shadow-[0_4px_24px_-12px_rgba(0,0,0,0.45)] transition hover:border-white/[0.1] hover:shadow-[0_8px_32px_-16px_rgba(250,204,21,0.06)] sm:p-5"
    >
      <div
      ...
      <div className="flex items-start gap-2 sm:gap-3">
        <span className="flex h-5 w-5 shrink-0 items-center justify-center rounded-md bg-gradient-to-br from-yellow-400/15 via-amber-500/10 to-slate-900/40 shadow-[inset_0_1px_0_0_rgba(250,204,21,0.12)] ring-1 ring-yellow-400/20">
          <Icon className="h-2.5 w-2.5 text-yellow-200/90" strokeWidth={2} aria-hidden />
        </span>
        <div className="min-w-0 flex-1">
          <div className="flex flex-wrap items-baseline gap-x-2 gap-y-1.5">
            <h3 className="shrink-0 text-[9px] font-semibold uppercase tracking-[0.14em] text-yellow-400/80 sm:text-[10px] sm:tracking-[0.16em]">
              {title}
            </h3>
            {items.map((s, i) => (
              <span
...
function SetupGrid({
  ...
  return (
    <div className={gridClassName}>
      ...
      />
      {subs.length > 0 ? (
        <div className="relative overflow-hidden rounded-2xl border border-yellow-400/15 bg-gradient-to-br from-yellow-400/[0.06] via-slate-950/40 to-transparent p-4 sm:col-span-2 sm:p-5">
          ...
          <div className="relative flex items-start gap-3">
            <span className="flex h-5 w-5 shrink-0 items-center justify-center rounded-md bg-gradient-to-br from-yellow-400/25 to-amber-900/40 ring-1 ring-yellow-400/25">
              <CookingPot className="h-2.5 w-2.5 text-yellow-200/90" strokeWidth={2} aria-hidden />
            </span>
            <div className="min-w-0 flex-1">
              <div className="flex flex-wrap items-baseline gap-x-2 gap-y-1.5">
                <h3 className="shrink-0 text-[9px] font-semibold uppercase tracking-[0.14em] text-yellow-300/85 sm:text-[10px] sm:tracking-[0.16em]">
                  {labels.substrates}
                </h3>
                {subs.map((r, i) => (
                  <span
                    key={i}
                    className="inline-flex max-w-full rounded-md border border-yellow-400/15 bg-slate-950/60 px-2 py-1 text-[11px] leading-snug text-slate-100 sm:text-xs"
                  >
                    {r.name}
                  </span>
                ))}
      ...
      <details className="group relative overflow-hidden rounded-2xl border border-white/[0.07] bg-gradient-to-br from-white/[0.04] to-transparent shadow-[0_8px_32px_-20px_rgba(0,0,0,0.5)] backdrop-blur-sm lg:hidden open:border-yellow-400/20 open:shadow-[0_12px_40px_-16px_rgba(250,204,21,0.08)]">
        <summary className="flex cursor-pointer list-none items-center gap-2 rounded-xl px-3 py-3 transition hover:bg-white/[0.03] [&::-webkit-details-marker]:hidden sm:gap-3">
          <span className="flex h-5 w-5 shrink-0 items-center justify-center rounded-lg bg-gradient-to-br from-yellow-400/25 via-amber-500/10 to-slate-900/50 shadow-[inset_0_1px_0_0_rgba(250,204,21,0.15)] ring-1 ring-yellow-400/25">
            <Wrench className="h-2.5 w-2.5 text-yellow-200/90" strokeWidth={1.75} aria-hidden />
          </span>
          ...
          <span className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full border border-white/[0.1] bg-black/30 text-slate-400 transition group-open:border-yellow-400/25 group-open:bg-yellow-400/10 group-open:text-yellow-200">
            <ChevronDown
